...
{
    [TestClass]
    public class MethodBeforeHandlingMessage
    {
        [TestMethod]
        public void Should_do_nothing_if_Consumer_is_not_PriorityBurrowConsumer()
        {
            ...
            // Action
            handler.BeforeHandlingMessage(null, new BasicDeliverEventArgs());
            handler.BeforeHandlingMessage(NSubstitute.Substitute.For<IBasicConsumer>(), new BasicDeliverEventArgs());
        }
        ...
        public void Should_send_notification_if_Consumer_is_PriorityBurrowConsumer()
        {
            ...
            var p = -1;
            sharedBroker.WhenAConsumerGetAMessage += (c, priority) =>
                                                     {
                                                         p = priority;
                                                     };
            var handler = new PriorityMessageHandler(NSubstitute.Substitute.For<IConsumerErrorHandler>(),
            ...
            // Action
            handler.BeforeHandlingMessage(consumer, eventArgs);
