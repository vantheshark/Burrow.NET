...
namespace Burrow.Tests.RPC.BurrowRpcLoadBalancerRouteFinderTests
{
    [TestClass]
    public class MethodFindExchangeName
    {
        [TestMethod]
        public void Should_return_noy_empty_string_if_type_is_RpcRequest()
        {
            // Arrange
            var routeFinder = new BurrowRpcLoadBalancerRouteFinder();

            // Action
            var exchangeName = routeFinder.FindExchangeName<RpcRequest>();

            // Assert
            Assert.AreEqual("Burrow.Exchange.Rpc.Requests", exchangeName);
        }
